...
class Xenia:
    def __init__(self):
        print("[X]> Welcome, I'm Xenia. How can I help you?")
    ...
    def evalClassify(self):
        ...
        print(str(self.classify) + " " + str(self.keywords))
    ...
    def answerRequest(self):
        ...
        if "s" in self.classify or ("k" in self.classify and len(self.classify) == 1 and len(self.keywords) != None):
            ...
            json.dump({"answer": answer, "classify": "s", "url": url}, io)
            return io.getvalue()
        elif "m" in self.classify:
            url = "https://www.google.de/maps/search/"+self.query.replace(" ", "%20")
            json.dump({"answer": answer, "classify": "m", "url": url},io)
            return io.getvalue()
        else:
            json.dump({"answer": answer},io)
            return io.getvalue()
